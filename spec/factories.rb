...
Factory.define :alt_new_customer, :class => Mousetrap::Customer, :default_strategy => :stub do |f|
  f.email { random_email_address }
  f.first_name { random_string }
  f.last_name { random_string }
  f.company { random_string }
  f.code { |me| me.email }
  f.add_attribute :id, nil
  f.subscription_attributes { Factory.attributes_for :i8l_subscription }
end
...
Factory.define :i8l_subscription, :class => Mousetrap::Subscription, :default_strategy => :stub do |f|
  f.plan_code 'TEST'
  f.billing_first_name { random_string }
  f.billing_last_name { random_string }
  f.credit_card_number '4111111111111111'
  ...
  f.billing_adddress "1 Upper Littleton\nWinford"
  f.city "Winford"
  f.state "Bristol"
  f.billing_zip_code 'NR25 7HG'
end
